...
if [ ! -f "/home/ark/.config/.update08182026" ]; then

  printf "\nFix standalone mednafen controls for OSH controller \n" | tee -a "$LOG_FILE"
  sudo rm -rf /dev/shm/*
  sudo wget --no-check-certificate https://github.com/slayer366/darkos-rg351p/raw/main/08182026/darkosupdate08182026.zip -O /home/ark/darkosupdate08182026.zip -a "$LOG_FILE" || rm -f /home/ark/darkosupdate08182026.zip | tee -a "$LOG_FILE"
  if [ -f "/home/ark/darkosupdate08182026.zip" ]; then
    sudo unzip -X -o /home/ark/darkosupdate08182026.zip -d / | tee -a "$LOG_FILE"
    sudo rm -v /home/ark/darkosupdate08182026.zip | tee -a "$LOG_FILE"
  else
    printf "\nThe update couldn't complete because the package did not download correctly.\nPlease retry the update again." | tee -a "$LOG_FILE"
    sleep 3
    echo $c_brightness > /sys/devices/platform/backlight/backlight/backlight/brightness
    ...
    printf "\nEnsure proper permissions are set for standalone mednafen \n" | tee -a "$LOG_FILE"
      sudo chmod 777 /usr/local/bin/mednafen
      sudo chmod 775 /home/ark/.mednafen/mednafen.cfg
...
if [ ! -f "/home/ark/.config/.update09212026" ]; then

  printf "\nUpdate LinApple with added OSK \n" | tee -a "$LOG_FILE"
  sudo rm -rf /dev/shm/*
  sudo wget --no-check-certificate https://github.com/slayer366/darkos-rg351p/raw/main/09212026/darkosupdate09212026.zip -O /home/ark/darkosupdate09212026.zip -a "$LOG_FILE" || rm -f /home/ark/darkosupdate09212026.zip | tee -a "$LOG_FILE"
  if [ -f "/home/ark/darkosupdate09212026.zip" ]; then
    sudo unzip -X -o /home/ark/darkosupdate09212026.zip -d / | tee -a "$LOG_FILE"
    sudo rm -v /home/ark/darkosupdate09212026.zip | tee -a "$LOG_FILE"
  else
    printf "\nThe update couldn't complete because the package did not download correctly.\nPlease retry the update again." | tee -a "$LOG_FILE"
    sleep 3
    echo $c_brightness > /sys/devices/platform/backlight/backlight/backlight/brightness
    ...
    printf "\nEnsure proper permissions are set for linapple \n" | tee -a "$LOG_FILE"
      sudo chmod ugo+rw /opt/linapple/gamecontrollerdb.txt
      sudo chmod ugo+rw /opt/linapple/res/osk.png
      sudo chmod 777 /opt/linapple/linapple
  ...
  touch "/home/ark/.config/.update09212026"

fi


if [ ! -f "/home/ark/.config/.update09222026" ]; then

  printf "\nUpdate LinApple with added OSK \n" | tee -a "$LOG_FILE"
  sudo rm -rf /dev/shm/*
  sudo wget --no-check-certificate https://github.com/slayer366/darkos-rg351p/raw/main/09222026/darkosupdate09222026.zip -O /home/ark/darkosupdate09222026.zip -a "$LOG_FILE" || rm -f /home/ark/darkosupdate09222026.zip | tee -a "$LOG_FILE"
  if [ -f "/home/ark/darkosupdate09222026.zip" ]; then
    sudo unzip -X -o /home/ark/darkosupdate09222026.zip -d / | tee -a "$LOG_FILE"
    sudo rm -v /home/ark/darkosupdate09222026.zip | tee -a "$LOG_FILE"
  else
    printf "\nThe update couldn't complete because the package did not download correctly.\nPlease retry the update again." | tee -a "$LOG_FILE"
    sleep 3
    echo $c_brightness > /sys/devices/platform/backlight/backlight/backlight/brightness
    ...
    printf "\nEnsure proper permissions are set for linapple \n" | tee -a "$LOG_FILE"
      ...
      sudo chmod ugo+rw /opt/linapple/res/osk.png
      sudo chmod 777 /opt/linapple/linapple
      sudo chmod 777 /usr/local/bin/apple2.sh
  ...
  touch "/home/ark/.config/.update09222026"
